...
type OptionsPayload = { drivers: Option[]; vehicles: Option[]; deliveries: Option[]; routes: Option[]; shifts: Option[]; users: Option[] };
const categories: Array<{ value: ReportCategory; title: string; description: string }> = [
  { value: "delivery", title: "Delivery Reports", description: "Delivery volume, status, assignments, routes, and exceptions." },
  { value: "driver", title: "Driver Performance", description: "Supported delivery activity, scheduled hours, availability, and ranking." },
  { value: "financial", title: "Financial Reports", description: "Revenue, expenses, maintenance costs, and net financial position." },
  { value: "operational", title: "Operational Reports", description: "Active work, route activity, schedules, assignments, and exceptions." },
  { value: "history", title: "Delivery Status History", description: "Review delivery status changes, timestamps, and recorded users." },
];
const expenseOptions = ["fuel", "maintenance", "repair", "insurance", "registration", "other"];
const deliveryStatuses = ["pending", "assigned", "in_transit", "delivered", "delayed", "failed", "returned"];
const vehicleStatuses = ["available", "assigned", "maintenance", "maintenance_due", "out_of_service"];
function monthStart() { const date = new Date(); return new Date(date.getFullYear(), date.getMonth(), 1).toISOString().slice(0, 10); }
function today() { return new Date().toISOString().slice(0, 10); }
function title(value: string) { return value.replaceAll("_", " ").replace(/\b\w/g, (character) => character.toUpperCase()); }
function display(value: unknown) { if (value === null || value === undefined || value === "") return "—"; if (typeof value === "number") return new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(value); return String(value); }
function csvCell(value: unknown) { return `"${String(value ?? "").replaceAll("\r\n", "\n").replaceAll("\r", "\n").replaceAll("\"", "\"\"")}"`; }

function ReportChart({ report }: { report: GeneratedReport }) {
  const chart = report.chart; if (!chart || !chart.labels.length || !chart.series.some((series) => series.values.some((value) => value !== 0))) return <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">No chart data is available for these criteria.</div>;
  const values = chart.series.flatMap((series) => series.values); const max = Math.max(1, ...values); const width = Math.max(560, chart.labels.length * 82); const height = 230;
  if (chart.type === "horizontal-bar") return <div className="space-y-3">{chart.labels.map((label, index) => <div className="grid items-center gap-3 sm:grid-cols-[180px_1fr_48px]" key={`${label}-${index}`}><p className="truncate text-xs font-semibold text-slate-600">{label}</p><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-purple-600" style={{ width: `${(chart.series[0]?.values[index] ?? 0) / max * 100}%` }} /></div><p className="text-right text-xs font-bold text-slate-700">{chart.series[0]?.values[index] ?? 0}</p></div>)}</div>;
...
export default function ReportsPage() {
  ...
  return <section className="reports-page space-y-5 text-[#17232b]"><AdminPageIntro description="Generate accurate operational reports from current DDMS data." eyebrow="Reporting center" title="Reports" />
    ...
    <AdminCard className="report-preview overflow-hidden"><div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><h2 className="text-lg font-semibold">Report Preview</h2><p className="mt-1 text-xs text-slate-500">{selectedCategory.title} · {reportTypes[category].find((item) => item.value === reportType)?.label}</p></div>{report ? <div className="report-actions flex gap-2"><SecondaryButton onClick={exportCsv} type="button">Export CSV</SecondaryButton><PrimaryActionButton onClick={() => window.print()} type="button">Print</PrimaryActionButton></div> : null}</div>{loading ? <div className="p-12 text-center text-sm font-semibold text-purple-700">Generating report...</div> : error ? <div className="p-10 text-center"><p className="text-sm font-semibold text-red-700">{error}</p><SecondaryButton className="mt-4" onClick={() => void generate()} type="button">Retry</SecondaryButton></div> : !report ? <div className="p-12 text-center"><p className="text-sm font-semibold text-slate-700">Select a category and report type, then generate a report to preview the results.</p></div> : <div className="p-5"><div className="print-brand hidden"><p className="text-lg font-bold">DeliverEaze Logistics</p></div><div className="flex flex-wrap justify-between gap-4"><div><h1 className="text-xl font-semibold">{report.title}</h1><p className="mt-1 text-sm text-slate-500">{report.period.from} to {report.period.to}</p>{report.filters.length ? <p className="mt-1 text-xs text-slate-500">Filters: {report.filters.map((item) => `${item.label}: ${item.value}`).join(" · ")}</p> : null}</div><p className="text-xs text-slate-400">Generated {new Date(report.generatedAt).toLocaleString()}</p></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{report.summary.map((item) => <div className="rounded-2xl bg-slate-50 p-4" key={item.label}><p className="text-xs text-slate-500">{item.label}</p><p className="mt-1 text-2xl font-bold">{item.value}</p>{item.context ? <p className="mt-1 text-[10px] text-slate-400">{item.context}</p> : null}</div>)}</div><div className="mt-6 rounded-2xl border border-slate-100 p-4"><ReportChart report={report} /></div>{report.rows.length ? <div className="mt-6 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr>{report.columns.map((column) => <th className="px-4 py-3 font-semibold" key={column.key}>{column.label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{report.rows.map((row, index) => <tr className="hover:bg-purple-50/30" key={index}>{report.columns.map((column) => <td className="whitespace-nowrap px-4 py-3 text-slate-600" key={column.key}>{display(row[column.key])}</td>)}</tr>)}</tbody></table></div> : <div className="mt-6 rounded-2xl border border-dashed border-slate-200 p-8 text-center"><p className="text-sm font-semibold text-slate-700">No records matched the selected report criteria.</p><p className="mt-1 text-xs text-slate-500">Try adjusting the date range or filters.</p></div>}</div>}</AdminCard>
    <style jsx global>{`@media print { body * { visibility: hidden !important; } .report-preview, .report-preview * { visibility: visible !important; } .report-preview { position: absolute !important; inset: 0 !important; border: 0 !important; box-shadow: none !important; } .report-preview > div:first-child, .report-actions, .report-controls, aside, nav { display: none !important; } .print-brand { display: block !important; margin-bottom: 16px; } @page { margin: 14mm; } }`}</style></section>;
}
