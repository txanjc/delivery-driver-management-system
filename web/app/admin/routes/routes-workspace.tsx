...
type RouteFocusSummary = { type: "driver" | "vehicle"; title: string; subtitle: string; routeIds: string[] };
...
function normalizeSearch(value: string) { return value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(); }
function compactLocation(value: string) { const parts = value.split(",").map((part) => part.trim()).filter(Boolean); return parts[0]?.replace(/^\d+\s+/, "") || value || "Unknown location"; }
function routeSummary(route: Pick<RouteRecord, "origin" | "destination">) { return `${compactLocation(route.origin)} -> ${compactLocation(route.destination)}`; }
function readableVehicle(vehicle?: VehicleRow) { return [vehicle?.vehicle_number, [vehicle?.make, vehicle?.model].filter(Boolean).join(" ")].filter(Boolean).join(" - ") || vehicle?.license_plate || "Unassigned vehicle"; }
function scoreMatch(query: string, values: string[], exactBoost = 0) {
  ...
  values.filter(Boolean).forEach((value) => {
    ...
    if (normalizedValue === normalizedQuery || compactValue === compactQuery) score = 0 - exactBoost;
    else if (normalizedValue.startsWith(normalizedQuery) || compactValue.startsWith(compactQuery)) score = 25;
    else if (normalizedValue.includes(normalizedQuery) || compactValue.includes(compactQuery)) score = 60;
...
function MapLegend() {
  ...
  const line = "h-1.5 w-8 rounded-full";
  return <div className={`${routesGlassDropdown} absolute bottom-16 right-20 z-30 p-2 text-[10px] text-slate-600`}><button className="flex w-full min-w-32 items-center justify-between gap-3 rounded-xl px-2 py-1.5 font-semibold text-slate-700" onClick={() => setOpen((value) => !value)} type="button"><span>Map legend</span><AppIcons.dropdown aria-hidden className={`transition ${open ? "rotate-180" : ""}`} size={13} weight="bold" /></button>{open ? <div className="mt-1 space-y-1.5 px-2 pb-1"><div className="flex items-center gap-2"><span className="grid h-4 w-4 place-items-center rounded-full border-2 border-purple-600 bg-white"><span className="h-1.5 w-1.5 rounded-full bg-purple-600" /></span>Origin</div><div className="flex items-center gap-2"><span className="grid h-4 w-4 place-items-center rounded-full bg-purple-600 text-[8px] text-white">⚑</span>Destination</div><div className="flex items-center gap-2"><span className={`${line} bg-purple-600`} />Active route</div><div className="flex items-center gap-2"><span className={`${line} bg-emerald-600`} />Completed</div><div className="flex items-center gap-2"><span className={`${line} bg-orange-500`} />Delayed / Exception</div><div className="flex items-center gap-2"><span className={`${line} bg-violet-700 ring-2 ring-purple-200`} />Selected</div></div> : null}</div>;
}
...
export function RoutesWorkspace() {
  ...
  const searchResults = useMemo(() => {
    ...
    deliveries.forEach((delivery) => {
      const rank = scoreMatch(query, [delivery.number, delivery.customer, delivery.phone, delivery.pickup, delivery.destination, delivery.status, delivery.priority], 40);
      if (rank === null) return;
      const route = routeByDeliveryId.get(delivery.id);
      results.push({ id: `delivery-${delivery.id}`, type: "delivery", title: delivery.number, subtitle: `${delivery.customer} - ${compactLocation(delivery.pickup)} -> ${compactLocation(delivery.destination)}`, metadata: delivery.priority, status: delivery.status, deliveryId: delivery.id, routeId: route?.id, rank });
    });
    routes.forEach((route) => {
      const rank = scoreMatch(query, [route.label, route.deliveryNumber, route.origin, route.destination, route.provider, route.customer], 25);
      if (rank === null) return;
      results.push({ id: `route-${route.id}`, type: "route", title: `Route for ${route.deliveryNumber}`, subtitle: routeSummary(route), metadata: `${formatDistance(route.distance)} - ${formatDuration(route.duration)}`, status: route.status, deliveryId: route.deliveryId, routeId: route.id, rank });
    });
    driverRecords.forEach((driver) => {
      const driverRoutes = routes.filter((route) => route.driverId === driver.id && routeIsActive(route));
      const allDriverRoutes = routes.filter((route) => route.driverId === driver.id);
      const rank = scoreMatch(query, [driver.name, driver.email, driver.phone, ...allDriverRoutes.map((route) => `${route.deliveryNumber} ${route.label}`)], 30);
      if (rank === null) return;
      results.push({ id: `driver-${driver.id}`, type: "driver", title: driver.name, subtitle: driver.email || driver.phone || "No contact recorded", metadata: `${driverRoutes.length} active route${driverRoutes.length === 1 ? "" : "s"}`, status: driver.availability, driverId: driver.id, rank });
    });
    vehicleRecords.forEach((vehicle) => {
      const vehicleRoutes = routes.filter((route) => route.vehicleId === vehicle.id && routeIsActive(route));
      const allVehicleRoutes = routes.filter((route) => route.vehicleId === vehicle.id);
      const rank = scoreMatch(query, [vehicle.number, vehicle.name, vehicle.makeModel, vehicle.plate, vehicle.type, vehicle.status, ...allVehicleRoutes.map((route) => `${route.deliveryNumber} ${route.label}`)], 35);
      if (rank === null) return;
      results.push({ id: `vehicle-${vehicle.id}`, type: "vehicle", title: [vehicle.number ? `Vehicle ${vehicle.number}` : "", vehicle.makeModel].filter(Boolean).join(" - ") || vehicle.name, subtitle: [vehicle.plate, vehicle.status].filter(Boolean).join(" - "), metadata: `${vehicleRoutes.length} active route${vehicleRoutes.length === 1 ? "" : "s"}`, status: vehicle.status, vehicleId: vehicle.id, rank });
    });
    routes.forEach((route) => {
      const originRank = scoreMatch(query, [route.origin]);
      if (originRank !== null) results.push({ id: `location-origin-${route.id}`, type: "location", title: compactLocation(route.origin), subtitle: `Origin for ${route.deliveryNumber}`, metadata: routeSummary(route), routeId: route.id, deliveryId: route.deliveryId, locationRole: "origin", rank: originRank + 12 });
      const destinationRank = scoreMatch(query, [route.destination]);
      if (destinationRank !== null) results.push({ id: `location-destination-${route.id}`, type: "location", title: compactLocation(route.destination), subtitle: `Destination for ${route.deliveryNumber}`, metadata: routeSummary(route), routeId: route.id, deliveryId: route.deliveryId, locationRole: "destination", rank: destinationRank + 12 });
    });
    const groupOrder: Record<RouteSearchResult["type"], number> = { delivery: 0, route: 1, driver: 2, vehicle: 3, location: 4 };
    return results.sort((left, right) => left.rank - right.rank || groupOrder[left.type] - groupOrder[right.type] || left.title.localeCompare(right.title));
  }, [deliveries, driverRecords, routeByDeliveryId, routes, search, vehicleRecords]);
  useEffect(() => {
    ...
    window.addEventListener("deliver-eaze:routes-search", updateSearch);
    return () => window.removeEventListener("deliver-eaze:routes-search", updateSearch);
  }, []);
  useEffect(() => {
    const requestId = searchRequestRef.current + 1;
    searchRequestRef.current = requestId;
    if (search.trim().length < 2) {
      window.dispatchEvent(new CustomEvent("deliver-eaze:routes-search-results", { detail: { loading: false, results: [] } }));
      return;
    }
    window.dispatchEvent(new CustomEvent("deliver-eaze:routes-search-results", { detail: { loading: true, results: [] } }));
    const timer = window.setTimeout(() => {
      if (searchRequestRef.current !== requestId) return;
      window.dispatchEvent(new CustomEvent("deliver-eaze:routes-search-results", { detail: { loading: false, results: searchResults } }));
    }, 300);
    return () => window.clearTimeout(timer);
  }, [search, searchResults]);
  useEffect(() => {
    function selectSearchResult(event: Event) {
      const result = (event as CustomEvent<RouteSearchResult>).detail;
      setFocusSummary(null);
      if (result.type === "delivery") {
        const route = result.routeId ? routes.find((item) => item.id === result.routeId) : routeByDeliveryId.get(result.deliveryId ?? "");
        if (route) { setSelectedId(route.id); setCardsVisible(true); return; }
        const delivery = deliveries.find((item) => item.id === result.deliveryId);
        if (delivery && delivery.pickupLatitude !== null && delivery.pickupLongitude !== null) mapRef.current?.centerOnPosition({ latitude: delivery.pickupLatitude, longitude: delivery.pickupLongitude });
        else if (delivery && delivery.destinationLatitude !== null && delivery.destinationLongitude !== null) mapRef.current?.centerOnPosition({ latitude: delivery.destinationLatitude, longitude: delivery.destinationLongitude });
        notify.info(`No generated route is available for ${delivery?.number ?? "this delivery"}.`);
        return;
      }
      if (result.type === "route" || result.type === "location") {
        const route = routes.find((item) => item.id === result.routeId);
        if (route) { setSelectedId(route.id); setCardsVisible(true); }
        return;
      }
      if (result.type === "driver") {
        const driverRoutes = routes.filter((route) => route.driverId === result.driverId && routeIsActive(route));
        if (driverRoutes.length === 1) { setSelectedId(driverRoutes[0].id); setCardsVisible(true); return; }
        if (driverRoutes.length > 1) { setSelectedId(""); setCardsVisible(true); setFocusSummary({ type: "driver", title: result.title, subtitle: `${driverRoutes.length} active routes`, routeIds: driverRoutes.map((route) => route.id) }); return; }
        notify.info(`No mapped routes are currently assigned to ${result.title}.`);
        return;
      }
      if (result.type === "vehicle") {
        const vehicleRoutes = routes.filter((route) => route.vehicleId === result.vehicleId && routeIsActive(route));
        if (vehicleRoutes.length === 1) { setSelectedId(vehicleRoutes[0].id); setCardsVisible(true); return; }
        if (vehicleRoutes.length > 1) { setSelectedId(""); setCardsVisible(true); setFocusSummary({ type: "vehicle", title: result.title, subtitle: `${vehicleRoutes.length} active routes`, routeIds: vehicleRoutes.map((route) => route.id) }); return; }
        notify.info(`No active routes are assigned to ${result.title}.`);
      }
    }
    window.addEventListener("deliver-eaze:routes-search-select", selectSearchResult);
    return () => window.removeEventListener("deliver-eaze:routes-search-select", selectSearchResult);
  }, [deliveries, notify, routeByDeliveryId, routes]);
  ...
  const visible = useMemo(() => routes.filter((route) => { const focusMatch = !focusSummary || focusSummary.routeIds.includes(route.id); const completed = showCompleted || route.status !== "completed"; const modeMatch = mode === "all" || mode === "deliveries" || (mode === "drivers" ? Boolean(route.driverId) : Boolean(route.vehicleId)); return focusMatch && completed && modeMatch; }), [focusSummary, mode, routes, showCompleted]); const selected = routes.find((route) => route.id === selectedId) ?? null;
  function selectRoute(route: RouteRecord) { setFocusSummary(null); setSelectedId(route.id); }
