import { apiError, authorizeAdministratorRequest, authorizeOperationsRequest } from "@/lib/server/administrator-api";
...
export async function GET(request: Request) {
  ...
  const [routesResponse, deliveriesResponse, driversResponse, vehiclesResponse, schedulesResponse, deliveryHistoryResponse, routeStopsResponse] = await Promise.all([
    authorization.client.from("routes").select("route_id, delivery_id, origin, destination, origin_name, origin_address, origin_latitude, origin_longitude, destination_name, destination_address, destination_latitude, destination_longitude, estimated_distance_km, estimated_duration_minutes, actual_distance_km, actual_duration_minutes, route_polyline, maps_url, route_provider, route_generated_at, sequence_order, created_at, route_number, route_date, driver_id, vehicle_id, schedule_id, start_location_name, start_address, start_place_id, start_latitude, start_longitude, end_location_name, end_address, end_place_id, end_latitude, end_longitude, return_to_depot, departure_time, shift_end_time, estimated_completion_time, total_distance_meters, total_duration_seconds, encoded_polyline, optimization_status, optimized_at, status").order("created_at", { ascending: false }),
  ...
  return Response.json({ routes: routesResponse.data ?? [], routeStops: routeStopsResponse.data ?? [], deliveries: deliveriesResponse.data ?? [], deliveryHistory: deliveryHistoryResponse.data ?? [], drivers, profiles: profilesResponse.data ?? [], vehicles: vehiclesResponse.data ?? [], schedules: schedulesResponse.data ?? [] });
}
...
export async function POST(request: Request) {
  ...
  if (error) return apiError(error.message, 400);
  if (validation.delivery?.assigned_driver_id) await notifyOperationalEvent(authorization.client, { type: "route_updated", key: `route:${data.route_id}:created`, title: "A route was assigned to you", message: "A route is ready for one of your assigned deliveries.", tone: "blue", badge: "Route assigned", module: "routes", relatedId: data.route_id, actionPath: notificationPaths.route(data.route_id), driverActionPath: `/route/${data.route_id}`, actionLabel: "View route", driverIds: [validation.delivery.assigned_driver_id], recipientRoles: ["driver"], details: [{ label: "Route date", value: new Date().toLocaleDateString("en-US") }, { label: "Stops", value: "2" }, { label: "Estimated distance", value: data.estimated_distance_km === null ? null : `${data.estimated_distance_km} km` }, { label: "Estimated duration", value: data.estimated_duration_minutes === null ? null : `${data.estimated_duration_minutes} minutes` }, { label: "Pickup", value: data.origin_address }, { label: "Destination", value: data.destination_address }] });
...
export async function PATCH(request: Request) {
  ...
  if (error) return apiError(error.message, 400);
  if (validation.delivery?.assigned_driver_id) await notifyOperationalEvent(authorization.client, { type: "route_updated", key: `route:${data.route_id}:${data.route_generated_at ?? "updated"}`, title: "Your route was updated", message: "Route details for an assigned delivery were updated.", tone: "blue", badge: "Route updated", module: "routes", relatedId: data.route_id, actionPath: notificationPaths.route(data.route_id), driverActionPath: `/route/${data.route_id}`, actionLabel: "View route", driverIds: [validation.delivery.assigned_driver_id], recipientRoles: ["driver"], details: [{ label: "Stops", value: "2" }, { label: "Estimated distance", value: data.estimated_distance_km === null ? null : `${data.estimated_distance_km} km` }, { label: "Estimated duration", value: data.estimated_duration_minutes === null ? null : `${data.estimated_duration_minutes} minutes` }, { label: "Pickup", value: data.origin_address }, { label: "Destination", value: data.destination_address }] });
